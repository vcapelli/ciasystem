...
export async function ehAdmin(db: D1Database, usuarioId: number): Promise<boolean> {
...
export async function buscarGrupoResponsavelProjetos(db: D1Database): Promise<number | null> {
...
export async function ehMembroAtivoDoGrupo(db: D1Database, usuarioId: number, grupoId: number): Promise<boolean> {
...
export async function podeVerProjetos(db: D1Database, usuarioId: number): Promise<boolean> {
...
export async function ehAdminDoGrupoResponsavel(db: D1Database, usuarioId: number): Promise<boolean> {
  if (await ehAdmin(db, usuarioId)) return true
  const grupoId = await buscarGrupoResponsavelProjetos(db)
  if (!grupoId) return false
