...
const paginas = new Hono<{ Bindings: Bindings }>()
...
// POST /paginas — cria página customizada, restrito a admin do sistema
...
// GET /paginas/:caminho — serve o conteúdo (checagem de acesso quando
// não é pública fica pro chamador informar usuario_id via query).
paginas.get('/:caminho', async (c) => {
  ...
  if (!pagina.publica) {
    if (!usuarioId) return c.json({ erro: 'login necessário' }, 401)
