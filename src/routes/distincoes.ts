...
export function criarRotaDistincao(tipo: TipoDistincao) {
  ...
  router.post('/', async (c) => {
    ...
    ).bind(body.titulo, body.descricao ?? null, body.imagem_url, usuarioId).run()
  ...
  router.post('/:id/conceder', async (c) => {
    ...
    const body = await c.req.json<{ usuario_id?: number; grupo_id?: number; motivo?: string }>()
    ...
    const concedidos = await conceder(c.env.DB, tipo, distincaoId, alvo, usuarioId, body.motivo)
